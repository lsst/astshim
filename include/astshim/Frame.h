...
public:
    ...
    */
    ...
    - @ref FrameSet
        ...
        If either of the "from" or "to" parameters is a pointer to a
        @ref FrameSet, then astConvert will attempt to convert from the
    ...
    @return A @ref FrameSet which describes the conversion and contains two Frames.
        ...
        Frame.  If the result is used as a Mappin\\
    ...
    */
    ...
    If called on a @ref FrameSet, the possibility exists that several of the Frames within
    it might be matched by the template.  Unless the choice is sufficiently restricted
    by the " domainlist" string, the sequence in which Frames are searched can then become
    important.  In this case, the search proceeds as follows:
    - Each field in the " domainlist" string is considered in turn.
    - An attempt is made to match the template to each of the target's Frames in the order:
        (1) the current Frame,
        (2) the base Frame,
        (3) each remaining Frame in the order of being added to the target @ref FrameSet.
    - Generally, the first match found is used. However, the Mapping between the target
        coordinate system and the resulting Frame is also examined. Preference is given
        to cases where both the forward and inverse transformations are defined
        (as indicated by the TranForward and TranInverse attributes). If only one transformation
        is defined, the forward one is preferred.
    ...
    - @ref FrameSet
        ...
        added to the target @ref FrameSet.
        ...
        attribute of this @ref FrameSet will be modified to identify
        the Frame whose match with the target was eventually accepted
