...
class TestObject(ObjectTestCase):
    ...
    @unittest.skip("FitsChan's Python interface does not yet support iteration")
    def test_FitsChanIteration(self):
        ss = astshim.StringStream("".join(self.cards))
        fc = astshim.FitsChan(ss)

        for card, descard in itertools.izip(fc, self.cards):
            self.assertEqual(card, descard)
    ...
    def test_FitsChanReadWrite(self):
        ...
        self.assertTrue(fv.found)
        self.assertEqual(fv.value.rstrip(), "WCSAXES =                    2 / Number of WCS axes")

        fv = fc2.findFits("%f", True)
        self.assertTrue(fv.found)
        self.assertEqual(fv.value.rstrip(), "CRPIX1  =                100.0 / Reference pixel on axis 1")

        fv = fc2.findFits("%f", True)
        self.assertTrue(fv.found)
        self.assertEqual(fv.value.rstrip(), "CRPIX2  =                100.0 / Reference pixel on axis 2")

        fv = fc2.findFits("%f", True)
        self.assertTrue(fv.found)
        self.assertEqual(fv.value.rstrip(), "CRVAL1  =                  0.0 / Value at ref. pixel on axis 1")

        fv = fc2.findFits("%f", True)
        self.assertTrue(fv.found)
        self.assertEqual(fv.value.rstrip(), "CRVAL2  =                  0.0 / Value at ref. pixel on axis 2")

        fv = fc2.findFits("%f", True)
        self.assertTrue(fv.found)
        self.assertEqual(fv.value.rstrip(), "CTYPE1  = 'RA---TAN'           / Type of co-ordinate on axis 1")

        fv = fc2.findFits("%f", True)
        self.assertTrue(fv.found)
        self.assertEqual(fv.value.rstrip(), "CTYPE2  = 'DEC--TAN'           / Type of co-ordinate on axis 2")

        fv = fc2.findFits("%f", True)
        self.assertTrue(fv.found)
        self.assertEqual(fv.value.rstrip(), "CDELT1  =                0.001 / Pixel size on axis 1")

        fv = fc2.findFits("%f", True)
        self.assertTrue(fv.found)
        self.assertEqual(fv.value.rstrip(), "CDELT2  =                0.001 / Pixel size on axis 2")
